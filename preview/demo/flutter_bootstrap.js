...
(function() {
  // 1. Allow the host page to override the load process if it defines a hook
  if (typeof window._flutter_load_entrypoint === 'function') {
    window._flutter_load_entrypoint(window._flutter);
    return;
  }

  // 2. Embedded mode: target #demo-host on the landing page
  const hostEl = document.getElementById('demo-host');
  if (hostEl) {
  ...
  // 3. Standalone mode: only mount to body if there is NO host page DOM (no header/main)
  const isHostPage = !!(document.querySelector('header') || document.querySelector('main'));
  if (!isHostPage) {
    _flutter.loader.load({
      config: {
    ...
    console.error("Flutter Web: '#demo-host' container not found. Aborting auto-load to prevent overwriting landing page DOM.");
  }
})();
